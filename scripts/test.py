...
parser.add_argument('--clevr_img_h5', type=str, default='/media/ssd/data/clevr_res101', help='dir of vqa image feature')
...
parser.add_argument('--resume', type=str, default='logs/clevr_kaiminginit_modClassifier_ResCNN_DPResOnly1BiLinear_res101_noShare_biLSTMword_AdvFCshift_d.0_resumeE6/model_epoch_9.pth', help='resume file name')
parser.add_argument('--logdir', type=str, default='logs/test', help='dir to tensorboard logs')
opt = parser.parse_args()
dataloader_test = getDataloader(opt)
...
def test(epoch):
	...
	for sample_batched in tqdm(dataloader_test):
		...
		pred = predicts.max(1)[1].data

		for i in range(opt.batch_size):
			answer_map[others[0][i]] = adict_rev[pred[i]]
